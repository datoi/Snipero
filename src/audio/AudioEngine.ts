...
const SOURCES: Record<SfxId, number> = {
  ...
  // Generated by tools/make-sfx.js rather than shipped with the pack.
  skill: require('../../assets/sfx/skill.wav'),
  skillReady: require('../../assets/sfx/skillReady.wav'),
};
...
const SPEC: Record<SfxId, SoundSpec> = {
  ...
  // A cast is rare and deliberate, so it earns the volume. The ready chime is
  // the opposite: it plays unprompted every cooldown for the whole run, so it
  // sits well under everything and is only ever noticed when it's useful.
  skill: { volume: 0.6, pool: 2, gap: 0.2 },
  skillReady: { volume: 0.22, pool: 1, gap: 0.5 },
};
